...
> 同じ罠を踏み直さないために、推測で直し始める前にここを見ること。
...
1. **フェーズ1「ドッキングせよ」**:
   - ムーンクレスタ完全再現のシアンピクセル文字で「**ドッキングせよ**」が画面上部に点滅表示！
   - 上空から3つのブロックが同時に降下（[1][2][3]やTab、クリックで切替）。
   - 左右の壁なし。自機に接触した瞬間に合体！スルーも可能。3つ落とし終えたら即座にバトルへ突入！
2. **フェーズ2「デストロイ ゼム オール！」**:
   - 同じピクセル文字で「**デストロイ　ゼム　オール！**」が赤色で画面上部に表示！
   - 8〜12機が一列に連なって優雅に流れるストリーム曲線大編隊（S字蛇行、8の字ループ、交差宙返りダイブ）。
   - 一網打尽に薙ぎ払う圧倒的爽快感！
3. **敵サイズの一回り拡大（迫力UP）**:
